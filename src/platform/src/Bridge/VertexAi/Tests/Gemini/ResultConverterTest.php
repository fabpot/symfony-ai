...
final class ResultConverterTest extends TestCase
{
    ...
    public function testStreamingConvertsChoicesToDeltas(array $chunk, string $expectedClass)
    {
        ...
        if ($items[$expectedIndex] instanceof TextDelta) {
            $this->assertSame('Hello', $items[$expectedIndex]->getText());
        }

        if ($items[$expectedIndex] instanceof BinaryDelta) {
            $this->assertSame('Hello', $items[$expectedIndex]->getData());
            $this->assertSame('text/plain', $items[$expectedIndex]->getMimeType());
        }

        if ($items[$expectedIndex] instanceof ToolCallComplete) {
            $this->assertSame('some_tool', $items[$expectedIndex]->getToolCalls()[0]->getName());
        }

        if ($items[$expectedIndex] instanceof ChoiceDelta) {
            $this->assertCount(2, $items[$expectedIndex]->getDeltas());
            $this->assertInstanceOf(TextDelta::class, $items[$expectedIndex]->getDeltas()[0]);
            $this->assertInstanceOf(ToolCallComplete::class, $items[$expectedIndex]->getDeltas()[1]);
        }
    }
